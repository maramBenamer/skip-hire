...
const SkipCard = ({ size, duration, price, allowsHeavy, allowedOnRoad, onSelect }) => {
  return (
    <div className="card">
      <div className="badge">{size}</div>
      <img src="./bin-image.jpg" alt="Skip" className="skip-img" />
      <h3>{size} Skip</h3>
      ...
      <button className="btn" onClick={() => onSelect({ size, duration, price })}>
        Select This Skip
      </button>
    </div>
  );
